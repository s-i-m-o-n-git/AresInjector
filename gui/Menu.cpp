...
#include "skStr.h"
#include <ShlObj.h> // Pour SHGetFolderPathA et CSIDL_*
// Forward declare message handler from imgui_impl_win32.cpp
extern IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
inline bool showUpdatePopup = false;
inline float popupStartTime = 0.0f;
bool Menu::initialize()
{
	...
	::RegisterClassEx(&wc);
	this->hwnd = ::CreateWindow(wc.lpszClassName, _T("Arès Injector"),
...
void Menu::loop()
{
	...
	while (this->isMenuOn)
	{
		...
		ImGui::Begin("Ares Client", 0, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
		...
		if (ImGui::Button("Update")) { // NE MARCHE PAS POUR L'INSTANT
			char appDataPath[MAX_PATH];
			if (SUCCEEDED(SHGetFolderPathA(NULL, CSIDL_APPDATA, NULL, 0, appDataPath))) {
				std::string dirPath = std::string(appDataPath) + "\\Arès";
				...
				popupStartTime = ImGui::GetTime();
			}
		}
		if (ImGui::Button("Exit Injector")) {
			
			
			
			ExitProcess(0); // Termine immédiatement le processus
		}
		// Popup affiché après l'update
		if (showUpdatePopup) {
			ImGui::OpenPopup("Mise a jour terminee");
			showUpdatePopup = false; // Pour ne pas l'ouvrir à chaque frame
		}

		if (ImGui::BeginPopupModal("Mise a jour terminee", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
			ImGui::Text("Files Update");
			ImGui::Separator();

			if (ImGui::Button("Fermer maintenant")) {
				ImGui::CloseCurrentPopup();
				popupStartTime = 0.0f;
			}

			// Ferme automatiquement au bout de 10 secondes
			if (ImGui::GetTime() - popupStartTime > 10.0f) {
				ImGui::CloseCurrentPopup();
				popupStartTime = 0.0f;
			}
